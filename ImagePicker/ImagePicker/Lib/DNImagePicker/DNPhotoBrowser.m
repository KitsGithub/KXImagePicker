...
- (void)updateNavigationBarAndToolBar {
    NSUInteger totalNumber = self.photosArray.count;
    ...
    if ([self.delegate respondsToSelector:@selector(photoBrowser:currentPhotoAssetIsSeleted:)]) {
        isSeleted = [self.delegate photoBrowser:self currentPhotoAssetIsSeleted:[self.photosArray objectAtIndex:self.currentIndex]];
    }
...
- (void)checkButtonAction
{
    if (self.checkButton.selected) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(photoBrowser:deseletedAsset:)]) {
            [self.delegate photoBrowser:self deseletedAsset:self.photosArray[self.currentIndex]];
            self.checkButton.selected = NO;
        }
    } else {
        if (self.delegate && [self.delegate respondsToSelector:@selector(photoBrowser:seletedAsset:)]) {
            self.checkButton.selected = [self.delegate photoBrowser:self seletedAsset:self.photosArray[self.currentIndex]];
        }
    }
...
- (void)sendButtonAction
{
    if ([self.delegate respondsToSelector:@selector(sendImagesFromPhotobrowser:currentAsset:)]) {
        [self.delegate sendImagesFromPhotobrowser:self currentAsset:self.photosArray[self.currentIndex]];
    }
}
